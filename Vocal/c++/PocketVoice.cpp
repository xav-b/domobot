...
int PocketVoice::loadConfigCommands(const string configFile) {
	...
	try {
        ...
        for ( ptree::iterator itRoot = pt.begin(); itRoot != pt.end(); itRoot++ ) {
            if ( itRoot->first == "commands" ) {
                ptree ptChild = pt.get_child("commands");
                // Useless at this moment
                for ( ptree::iterator itChild = ptChild.begin(); itChild != ptChild.end(); itChild++ ) {
                    std::cout << "Read: " << itChild->first << " -> " << itChild->second.data() << std::endl;
                    commands[itChild->first] = itChild->second.data();
                }
            } 
        }
    }
...
std::string PocketVoice::recognize() {
    int16 adbuf[4096];
    int32 k, ts, rem;
    char const *hyp;
    char const *uttid;
...
int PocketVoice::execute(string said) {
    int ret(-1);
    std::cout << "Executing mapped command...\n";
    if ( system(NULL) ) {
        std::cout << "Processor available, running: " << commands[said] << std::endl;
        ret = system( commands[said].c_str() );
    }
    else
        return 1;
    return ret;
...
int PocketVoice::synthetize(string sentence) {
    int ret(-1);
    string tts_cmd = "../tts/googleTTS.py " + sentence;
    std::cout << "Sending synthetize request to google...\n";
    if ( system(NULL) ) {
        std::cout << "Processor available, processing: " << sentence << std::endl;
        ret = system( tts_cmd.c_str() );
    }
    else
        return 1;
    return ret;
